...
La sintaxis mas facil de comprender como usuario en personal fue Scala y Python, tiene la sintaxis más fácil de comprender y escribir, especialmente para usuarios que están aprendiendo o quieren enfocarse en la lógica sin preocuparse tanto por las estructuras.
...
Documentacion y ejercicio de Scala, Haskell y Python
Scala
...
Scala tiene soporte nativo para la programación funcional, por eso las operaciones como map, filter y reduce se aplican directamente sobre las listas.
Su sintaxis es más concisa y expresiva que la de Java, permitiendo escribir funciones anónimas con => y usar tuplas fácilmente.
...
La función getAverage toma una lista de enteros como entrada, suma todos sus elementos utilizando sum nums y obtiene la cantidad de elementos con length nums.
Luego, convierte ambos valores enteros a tipo Double mediante fromIntegral, para realizar una división real (no entera) y obtener un resultado decimal que corresponde al promedio.
...
Documentacion
Python también permite usar funciones funcionales con map(), filter() y reduce(), junto con funciones lambda para definir operaciones en una sola línea.
Gracias a su sintaxis simple, el código es fácil de leer y mantener.
